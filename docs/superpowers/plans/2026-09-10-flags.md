...
- Produces: `func (c *Capture) Flags() *FlagStore`; `func (c *Capture) MarkNow() (uint64, bool)`; `func flagsForWindow(marks []uint64, start, end uint64) []Flag`.
...
`save_test.go` imports only `os`, `path/filepath`, `testing` and `time` today.
These tests also need `github.com/gabeduke/hindsight/internal/config`; add it.

- [ ] **Step 2: Run the tests to verify they fail**

Run: `go test ./internal/audio/ -run 'FlagsForWindow|MarkNow|MarkPlacedNow' -v`
...
Add the accessor and the marking entry point beside `func (c *Capture) Ring()`:
...
Run: `go test ./internal/audio/ -run 'FlagsForWindow|MarkNow|MarkPlacedNow' -v && go build ./... && go test ./internal/audio/ -race`
Expected: PASS (6 new tests), whole package still green.
...
```js
**Read `web/static/lib/takes.js` first.** The real shapes, which this task uses
rather than inventing parallel ones:

- `this.rows` is a `Map` keyed by take name; `this.rows.get(name)` finds a row.
- `row.data` holds the take object; `row.waveEl` is the `.wave` container.
- **`this.patchTake(name, patch)` already exists.** It PATCHes, throws on
  failure, clears the etag and calls `refresh()`, so the list reconciles itself
  after a write. Use it — do not write a second `fetch`.
- `mountWave(row)` mounts the waveform and is guarded by `row.ws`/`row.mounting`.

Add to the `Takes` class:
...
```js
  ...
  async setFlags(row, flags) {
    try {
      // patchTake refreshes the list, which re-renders this row's flags.
      await this.patchTake(row.name, { flags });
    } catch (e) {
      this.onError?.(e.message || 'could not update flags');
    }
  }
```

Call `this.renderFlags(row)` from the same place the row's other fields are
applied from `row.data`, so a refresh redraws the ticks. Use whatever the file's
existing error surface is in place of `this.onError?.()` if it differs.
...
Attach this **once, where the row element is built** (the block that assigns
`nameEl`, `waveEl`, `playBtn` and friends), not in `mountWave` — `mountWave` can
run more than once per row and would stack duplicate handlers.
